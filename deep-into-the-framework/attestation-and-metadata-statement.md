...
The Metadata Statements are issued by the manufacturer of the authenticators. These statements contain all the necessary information to verify the Attestation Statements generated by the authenticators during the attestation ceremony.

There are several possible sources to get theses Metadata Statements. The main source is the [FIDO Alliance Metadata Service](https://fidoalliance.org/metadata) that allows to fetch statements on-demand.
...
## Fetching Attestation Statement

### Attestation Metadata Repository

First of all, you must prepare an Attestation Metadata Repository. This service will manage all Metadata Statements depending on there source \(local storage or distant\).
